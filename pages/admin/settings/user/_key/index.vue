      ...
      async getData () {
        try {
          ...
          if (routeParams instanceof Object && routeParams.key !== 'new') {
            ...
            let data = await this.$axios.$get(this.$store.state.apiUrl + '/admin/user/' + routeParams.key)
            console.log(data)
      ...
      async saveData () {
        try {
          ...
          if (!this.checkErrors) {
            ...
            let data = ''
            if (!this.isNew) {
              data = await this.$axios.$put(this.$store.state.apiUrl + '/admin/user', postData)
            } else {
              data = await this.$axios.$post(this.$store.state.apiUrl + '/admin/user', postData)
            }
            console.log(data)
            this.isLoading = false
